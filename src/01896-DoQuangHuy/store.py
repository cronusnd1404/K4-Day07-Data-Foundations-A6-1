...
class EmbeddingStore:
    ...
    def search_with_filter(self, query: str, top_k: int = 3, metadata_filter: dict = None) -> list[dict]:
        ...
        """
        if self._use_chroma and self._collection is not None:
            return self._query_chroma(query, top_k, metadata_filter)
    ...
    def _query_chroma(self, query: str, top_k: int, metadata_filter: dict | None = None) -> list[dict]:
        query_embedding = self._embedding_fn(query)
        ...
        return [
            {"id": doc_id, "content": content, "metadata": metadata, "score": -distance}
