...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        for i in range(0, len(raw_sentences), self.max_sentences_per_chunk):
            chunk = " ".join(raw_sentences[i : i + self.max_sentences_per_chunk]).strip()
            if chunk:
                chunks.append(chunk)
        return chunks
...
class RecursiveChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        return [chunk.strip() for chunk in self._split(normalized_text, self.separators) if chunk.strip()]
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for index, part in enumerate(parts):
            if not part:
                continue
            chunk_text = part + sep if index < len(parts) - 1 else part
            chunk_text = chunk_text.strip()
            if len(chunk_text) <= self.chunk_size:
                chunks.append(chunk_text)
            else:
                chunks.extend(self._split(chunk_text, remaining_separators[1:]))
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        return {name: summarize(chunks) for name, chunks in strategies.items()}
